...
if (!(has_admin_access_level() || has_pilot_access_level()  || has_representative_access_level('sfx19'))) {
    echo "Access Denied";
    die();
}
...
if (isset($_POST['id_user'])) {
    ...
    $isrep = $bdd->select("SELECT 1 from representative where id_user = {$id_user};");
    if (!$isrep) {
        echo "Update Failed: Not A Representative.";
        die();
    }
    // --------------------------------------------------
    ...
    $success = $bdd->update(
        ...
        UPDATE representative SET lastname = :lastname, firstname = :firstname, id_center = :id_center WHERE id_user = :id_user;", $binds);
